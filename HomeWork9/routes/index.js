...
router.post('/search', function(req, res, next) {
  MongoClient.connect(url, function(err, db) {
     ...
     db.collection('HW9').find({'location':{'$near':{'$geometry':{'type': 'point', 'coordinates': coord}, '$maxDistance':20000}}}).forEach(function(doc){
...
router.post('/save', function(req, res, next) {
   MongoClient.connect(url, function(err, db) {
            ...
            db.collection('HW9').insert({name:req.body.name, category:req.body.category,location:[long, lat]},function(err){
                 if(err) throw err;
                 req.body.name = '';
                 req.body.lat = '';
                 req.body.long = '';
